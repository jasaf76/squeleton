...
const CourseCard = ({
    ...
    return (
      <>
        {data ? (
          <Link
            to={`/curso/detalle/${data.course_uuid}`}
            onMouseEnter={() => setEffectCard(true)}
            onMouseLeave={() => setEffectCard(false)}
            className={`${
              effectCard && "animate-card "
            } w-64 h-auto rounded-md dark:bg-dark-third dark:hover:bg-dark-main shadow-sm dark:border-none border hover:shadow-md bg-white transition duration-300 ease-in-out cursor-pointer m-2  font-regular p-1 inline-block`}>
            <div className="w-full h-36 mr-4 rounded transition duration-200 ease-in-out overflow-hidden md:h-40">
              <video
                loop
                muted
                className="object-cover w-full h-full "
                src={`${process.env.REACT_APP_API_URL}${data.sales_video}`}
                poster={`${process.env.REACT_APP_API_URL}${data.thumbnail}`}
                onMouseOver={(event) => event.target.play()}
                onMouseOut={(event) => event.target.load()}></video>
            </div>
            <div className="mt-1 ml-2">
              <div className="flex">
                <div className="mr-4 flex-shrink-0">
                  <img
                    ...
                    src={`${process.env.REACT_APP_API_URL}${data.author.get_picture}`}
                    alt=""
                  />
                </div>
                <div>
                  <h4 className="text-sm font-gilroy-bold dark:text-dark-txt">
                  ...
                  <p className="mt-1 font-gilroy-regular dark:text-dark-txt">
                    <span className="truncate text-sm">
                      {data.author.username.length > 27
                        ? data.author.username.slice(0, 26) + "..."
                        : data.author.username}
                    </span>
                    {data.author && data.author.verified ? (
                      <BadgeCheckIcon
                        className="ml-1 h-4 w-4 text-blue-500 inline-flex"
                        aria-hidden="true"
                      />
                    ) : (
                      <></>
              ...
              <h3 className="text-sm text-yellow-600 my-1">
                <span className="text-yellow-600 font-gilroy-semibold inline-flex mr-1">
                  {data.student_rating}
                </span>
                <span className="inline-flex mr-2">
                  <Stars rating={data.student_rating} />
                </span>
                <span className="dark:text-dark-txt text-gray-400 text-xs inline-flex">
